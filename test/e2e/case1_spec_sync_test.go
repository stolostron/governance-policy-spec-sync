...
var _ = Describe("Test spec sync", func() {
	BeforeEach(func() {
		...
		output, err = utils.KubectlWithOutput("kubectl get namespace")
		fmt.Println(string(output))
		Expect(err).Should(BeNil())
		output, err = utils.KubectlWithOutput("kubectl get policy.policy.open-cluster-management.io/default.case1-test-policy status")
		fmt.Println(string(output))
		Expect(err).Should(BeNil())
	...
	AfterEach(func() {
		...
		Expect(err).Should(BeNil())
		fmt.Println(string(output))
		output, err = utils.KubectlWithOutput("kubectl get policy.policy.open-cluster-management.io/default.case1-test-policy")
		fmt.Println(string(output))
		Expect(err).Should(BeNil())
		output, err = utils.KubectlWithOutput("kubectl get policy.policy.open-cluster-management.io/default.case1-test-policy status")
		fmt.Println(string(output))
